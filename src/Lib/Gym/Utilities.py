...
def Get_Robot_Structure_Theta_Home(name: str, Env_ID: int) -> tp.List[float]:
    ...
    try:
        ...
        if Env_ID == 0:
            ...
            return {
                'Universal_Robots_UR3': Mathematics.Degree_To_Radian(np.array([-92.02061, -103.58287, -83.66714, -82.6988, 89.83795, -1.75638], dtype=np.float64)),
                'ABB_IRB_120': Mathematics.Degree_To_Radian(np.array([-14.9314, 18.78947, 7.9967, -0.00011, 63.21423, -14.93134], dtype=np.float64)),
                'ABB_IRB_120_L_Ax': np.append([0.30549], Mathematics.Degree_To_Radian(np.array([89.16176, 16.09583, 11.41534, 0.0, 62.48923, -0.83837], dtype=np.float64))),
