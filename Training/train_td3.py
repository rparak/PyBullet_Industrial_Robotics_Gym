...
def main():
    ...
    action_noise = stable_baselines3.common.noise.NormalActionNoise(mean=np.zeros(n_action), 
    ...
    # ...
    if CONST_ALGORITHM_NAME == 'TD3':
        model = stable_baselines3.TD3(policy="MultiInputPolicy", env=gym_environment, gamma=0.95, learning_rate=0.001, action_noise=action_noise, device='cuda', 
                                      batch_size=256, policy_kwargs=dict(net_arch=[256, 256, 256]), verbose=1)
    elif CONST_ALGORITHM_NAME == 'TD3_HER':
        model = stable_baselines3.TD3(policy="MultiInputPolicy", env=gym_environment, replay_buffer_class=stable_baselines3.HerReplayBuffer, gamma=0.95, learning_rate=0.001, action_noise=action_noise, device='cuda', 
